...
export class CustomDatePickerAdapter extends NativeDateAdapter {
  ...
  override format(date: Date, display: string | DateDisplay): string  {
    ...
    if (display === 'customInput') {
      return new DatePipe(Object.values(this.locale)[0] as string).transform(date, 'shortDate') || '';
    } else {
      return  new DatePipe(Object.values(this.locale)[0] as string).transform(date, 'MMM yyyy') || '';
    }
  }
}
